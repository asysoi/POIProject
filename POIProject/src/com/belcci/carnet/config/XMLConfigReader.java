...
public class XMLConfigReader extends ConfigReader {
	...
	public static ConfigReader getInstance(String configFileName) {
		if (reader == null) {
	...
	public CountryList getCountryList() {
		...
		if (doc != null) {
			NodeList countries = doc.getElementsByTagName(TAG_COUNTRIES);

			for (int i = 0; i < countries.getLength(); i++) {
				NodeList childs = countries.item(i).getChildNodes();

				for (int j = 0; j < childs.getLength(); j++) {
					Node item = childs.item(j);

					if (TAG_COUNTRY.equals(item.getNodeName())) {
						Country country = getCountry(item);
						country.setId(Integer.parseInt(item.getAttributes()
								.getNamedItem(TAG_ID).getNodeValue()));
						clist.add(country);
					}
				}
		...
		for (int i = 0; i < tags.getLength(); i++) {
			...
			if (item.getNodeType() == Element.ELEMENT_NODE) {
				...
				if (TAG_CODE.equals(tagname)) {
					country.setCode(tagvalue);
				} else if (TAG_NAME.equals(tagname)) {
					country.setName(tagvalue);
				} else if (TAG_ENAME.equals(tagname)) {
					country.setEname(tagvalue);
				}
			}
		}
		return country;
	...
	public JobList getJobList() {
		JobList jlist = JobList.getInstance();

		if (doc != null) {
			NodeList jobs = doc.getElementsByTagName(TAG_JOBS);

			for (int i = 0; i < jobs.getLength(); i++) {
				NodeList childs = jobs.item(i).getChildNodes();

				for (int j = 0; j < childs.getLength(); j++) {
					Node item = childs.item(j);

					if (TAG_JOB.equals(item.getNodeName())) {
						Job job = getJob(item);
						jlist.add(job);
					}
				}
		...
		for (int i = 0; i < tags.getLength(); i++) {
			...
			if (item.getNodeType() == Element.ELEMENT_NODE) {
				...
				if (TAG_NAME.equals(tagname)) {
					job.setName(tagvalue);
				} else if (TAG_ENAME.equals(tagname)) {
					job.setEname(tagvalue);
				} else if (TAG_ID.equals(tagname)) {
					job.setId(Integer.parseInt(tagvalue));
				}
			}
		}
		return job;
	...
	public TargetList getTargetList() {
		TargetList clist = TargetList.getInstance();
		clist.clear();
		
		if (doc != null) {
			NodeList elements = doc.getElementsByTagName(TAG_TARGETS);
			...
			for (int i = 0; i < elements.getLength(); i++) {
				...
				for (int j = 0; j < childs.getLength(); j++) {
					...
					if (TAG_TARGET.equals(item.getNodeName())) {
						Target element = getTarget(item);
						element.setId(Integer.parseInt(item.getAttributes()
								.getNamedItem(TAG_ID).getNodeValue()));
						clist.add(element);
					}
				}
	...
	private Target getTarget(Node node) {
		...
		for (int i = 0; i < tags.getLength(); i++) {
			...
			if (item.getNodeType() == Element.ELEMENT_NODE) {
				...
				if (TAG_NAME.equals(tagname)) {
					element.setName(tagvalue);
				} else if (TAG_ENAME.equals(tagname)) {
					element.setEname(tagvalue);
				}
			}
		}
		return element;	
	...
	public ProductTypeList getProductTypeList() {
		ProductTypeList clist = ProductTypeList.getInstance();
		clist.clear();
		
		if (doc != null) {
			NodeList elements = doc.getElementsByTagName(TAG_TYPES);
			...
			for (int i = 0; i < elements.getLength(); i++) {
				...
				for (int j = 0; j < childs.getLength(); j++) {
					...
					if (TAG_TYPE.equals(item.getNodeName())) {
						ProductType element = getProductType(item);
						element.setId(Integer.parseInt(item.getAttributes()
								.getNamedItem(TAG_ID).getNodeValue()));
						clist.add(element);
					}
	...
	private ProductType getProductType(Node node) {
